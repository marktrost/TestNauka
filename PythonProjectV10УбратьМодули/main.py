...
# --- Редактировать блок ---
@app.route("/admin/edit_block/<int:block_id>", methods=["POST"])
...
def edit_subject(subject_id):
    ...
    # Получаем вариант для хлебных крошек
    variant = conn.execute("SELECT * FROM variants WHERE id=?", (subject['variant_id'],)).fetchone()
    ...
    conn.close()
    return render_template("edit_subject.html", subject=subject, variant=variant)
